...
<div class="admin-card" style="max-width: 800px; margin: 0 auto;">
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:1.25rem;">
        @csrf
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="client_name" name="client_name" value="{{ old('client_name') }}" class="form-control @error('client_name') error @enderror" placeholder="e.g. Fatima Al-Hassan" required>
                @error('client_name')
            ...
            <div class="form-group">
                ...
                <input type="text" id="company" name="company" value="{{ old('company') }}" class="form-control @error('company') error @enderror" placeholder="e.g. ShopEase Ltd.">
                @error('company')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="position" name="position" value="{{ old('position') }}" class="form-control @error('position') error @enderror" placeholder="e.g. Founder & CTO">
                @error('position')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <textarea id="review" name="review" rows="5" class="form-control @error('review') error @enderror" placeholder="Copy-paste client comments or feedback details..." required>{{ old('review') }}</textarea>
            @error('review')
            <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="display:flex; justify-content:flex-end; gap:1rem; border-top:1px solid #E2E8F0; padding-top:1.25rem; margin-top:.5rem;">
            <a href="{{ route('admin.testimonials.index') }}" class="btn" style="border:1px solid #E2E8F0; color:#0F172A; background:transparent;" id="cancel-testimonial-btn">Cancel</a>
            <button type="submit" class="btn btn-primary" id="save-testimonial-btn">Create Testimonial</button>
        </div>
