...
<div class="admin-card" style="max-width: 850px; margin: 0 auto;">
    ...
    <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:1.25rem;">
        @csrf
        ...
        <div style="display:grid; grid-template-columns: 2fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" class="form-control @error('title') error @enderror" placeholder="e.g. Understanding Core Web Vitals in Laravel 12" required>
                @error('title')
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="author" name="author" value="{{ old('author', 'Brother IT Digital') }}" class="form-control @error('author') error @enderror">
                @error('author')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="tags" name="tags" value="{{ old('tags') }}" class="form-control @error('tags') error @enderror" placeholder="e.g. Laravel, PHP, Speed Optimization">
                @error('tags')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <textarea id="excerpt" name="excerpt" rows="2" class="form-control @error('excerpt') error @enderror" placeholder="Write a summary sentence to display in lists...">{{ old('excerpt') }}</textarea>
            @error('excerpt')
            <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="content" name="content" rows="10" class="form-control @error('content') error @enderror" placeholder="Write the full body content of your article here (HTML allowed)..." required>{{ old('content') }}</textarea>
            @error('content')
            <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1.5fr; gap:1rem; align-items:center;">
            ...
            <div style="display:flex; gap:2rem; margin-top:1rem;">
                ...
                <div style="display:flex; align-items:center; gap:.5rem;">
                    <input type="checkbox" id="is_published" name="is_published" value="1" {{ old('is_published') ? 'checked' : '' }} style="width:1.1rem; height:1.1rem; cursor:pointer;">
                    <label for="is_published" style="font-weight:600; cursor:pointer; margin:0;">Publish Immediately</label>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <textarea id="meta_description" name="meta_description" rows="2" class="form-control" placeholder="Search result description tag fallback"></textarea>
            </div>
        </div>

        {{-- Form Actions --}}
        <div style="display:flex; justify-content:flex-end; gap:1rem; border-top:1px solid #E2E8F0; padding-top:1.25rem; margin-top:.5rem;">
            <a href="{{ route('admin.blog.index') }}" class="btn" style="border:1px solid #E2E8F0; color:#0F172A; background:transparent;" id="cancel-blog-btn">Cancel</a>
            <button type="submit" class="btn btn-primary" id="save-blog-btn">Create Article</button>
        </div>
