...
<div class="admin-card" style="max-width: 800px; margin: 0 auto;">
    ...
    <form action="{{ route('admin.portfolios.store') }}" method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:1.25rem;">
        @csrf
        ...
        <div style="display:grid; grid-template-columns: 1.5fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" class="form-control @error('title') error @enderror" placeholder="e.g. ShopEase E-commerce Platform" required>
                @error('title')
            ...
            <div class="form-group">
                ...
                <select id="category" name="category" class="form-control @error('category') error @enderror" style="height:auto;" required>
                    ...
                    <option value="wordpress_landing" {{ old('category') === 'wordpress_landing' ? 'selected' : '' }}>Wordpress Landing Page</option>
                    <option value="custom_code_landing" {{ old('category') === 'custom_code_landing' ? 'selected' : '' }}>Custom Code Landing Page</option>
                </select>
                @error('category')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="client" name="client" value="{{ old('client') }}" class="form-control @error('client') error @enderror" placeholder="e.g. ShopEase Ltd.">
                @error('client')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" id="completed_at" name="completed_at" value="{{ old('completed_at') }}" class="form-control @error('completed_at') error @enderror">
                @error('completed_at')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <div style="display:flex; gap:0.5rem;">
                    <input type="url" id="live_url" name="live_url" value="{{ old('live_url') }}" class="form-control @error('live_url') error @enderror" placeholder="https://example.com" style="flex:1;">
            ...
            <div class="form-group">
                ...
                <input type="url" id="github_url" name="github_url" value="{{ old('github_url') }}" class="form-control @error('github_url') error @enderror" placeholder="https://github.com/user/repo">
                @error('github_url')
                <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="technologies" name="technologies" value="{{ old('technologies') }}" class="form-control @error('technologies') error @enderror" placeholder="e.g. Laravel, React, Tailwind CSS, MySQL">
            @error('technologies')
            <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="short_description" name="short_description" rows="3" class="form-control @error('short_description') error @enderror" placeholder="Short overview sentence..." required>{{ old('short_description') }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="6" class="form-control @error('description') error @enderror" placeholder="Provide a detailed case study or project explanation...">{{ old('description') }}</textarea>
            @error('description')
            <span style="color:#EF4444; font-size:.75rem; margin-top:.25rem; display:block;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="display:flex; justify-content:flex-end; gap:1rem; border-top:1px solid #E2E8F0; padding-top:1.25rem; margin-top:.5rem;">
            <a href="{{ route('admin.portfolios.index') }}" class="btn" style="border:1px solid #E2E8F0; color:#0F172A; background:transparent;" id="cancel-project-btn">Cancel</a>
